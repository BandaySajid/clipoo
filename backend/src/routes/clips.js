...
import { pushClipToCache, removeClipFromCache } from '../lib/redis.js';
...
app.post('/', async (c) => {
    ...
    if (body.type === 'IMAGE' && body.content?.startsWith('data:')) {
        const [, meta, b64] = body.content.match(/^data:([^;]+);base64,(.+)$/) || [];
        if (meta && b64) {
            const ext = meta.split('/')[1] || 'png';
            r2_key = `images/${id}.${ext}`;
            const buffer = Buffer.from(b64, 'base64');
            content = await uploadToR2(r2_key, buffer, meta);
        }
    }

    const clip = { id, type: body.type, content, r2_key: r2_key || null, device: body.device, created_at, room };
    ...
    d1Query(
        ...
        [id, clip.type, clip.content, clip.r2_key, clip.device, created_at, room]
